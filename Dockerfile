...
CMD mkcert -install && \
    for i in $(echo $domain | sed "s/,/ /g"); do mkcert $i; done && \
    for i in $(echo $domain | sed "s/,/ /g"); do cat $i-key.pem $i.pem | openssl pkcs12 -export -out $i.p12 -password pass:password; done && \
    for i in $(echo $domain | sed "s/,/ /g"); do keytool -importkeystore -srckeystore $i.p12 -srcstoretype pkcs12 -destkeystore $i.jks -srcstorepass password -deststorepass password; done
